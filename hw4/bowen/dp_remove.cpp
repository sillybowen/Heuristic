...
bool DPRemove::tippedBoard(int stat) {
  ...
  while (tmp>0) {
    if (tmp%2==1) {
      f1+= (12-positions_[index])*weights_[index];
      f2+= (positions_[index]-14)*weights_[index];
    }
    tmp/=2;
    index--;
  }
...
// if the state is not must win state
  action[bitN] = -1;
  if (!winStat[bitN] && ((degree%2==0 && !start_at_odd )||
                         (degree%2==1 && start_at_odd))) {
    for (int i = 0; i<board_.size();i++)
      if (board_[i] !=0) {
        if (action[bitN]==-1)
          action[bitN] = i;
