package voronoi.utils;

public final class Constants {
  /*
   * JSON attributes for class "GetPackerEventWithAssign" change all those
   * String objects to an JSON object, then convert to "event" string
   */
  public final static int WIDTH_OF_SQUARE = 2 * 1000;
  public final static int INIT_WINDOW_WIDTH = 860;
  public final static int INIT_WINDOW_LENGTH = 750;
  public final static double INIT_GAME_ZOOM_LEVEL = 0.65 / 2.0;
  public final static int RIGHT_CONTROL_PANEL_WIDTH = 180;

  public final static int DEFAULT_GAME_PORT = 5678;
  public final static int MAX_NUM_PLAYERS = 2;
  public final static int MAX_NUM_PLAYERS2 = 3;
  public final static int NUM_OF_STONES_EACH = 5;
}
